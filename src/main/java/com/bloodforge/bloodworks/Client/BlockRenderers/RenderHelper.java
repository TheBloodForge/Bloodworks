...
public class RenderHelper
{
    ...
    {
        renderCuboid(matrix, vertexBuilder, pos, size, sizeShrink, minU, minV, maxU, maxV, light, 1f, 1f, 1f, 1f, true, true, true, true, true, true, drawInside);
    }

    public static void renderCuboidWithDifferentSideUVs(Matrix4f matrix, VertexConsumer vertexBuilder, Vec3 pos, Vec3 size, float sizeShrink, float minU, float minV, float maxU, float maxV, float minUs, float minVs, float maxUs, float maxVs, int light, boolean drawInside)
    {
        renderCuboidWithDifferentSideUVs(matrix, vertexBuilder, pos, size, sizeShrink, minU, minV, maxU, maxV, minUs, minVs, maxUs, maxVs, light, 1f, 1f, 1f, 1f, true, true, true, true, true, true, drawInside);
    }
    ...
    {
        renderCuboid(matrix, vertexBuilder, pos, size, sizeShrink, minU, minV, maxU, maxV, light, 1f, 1f, 1f, 1f, drawN, drawE, drawS, drawW, drawU, drawD, drawInside);
    }
                                    ...
                                    float r, float g, float b, float alpha, boolean drawN, boolean drawE, boolean drawS, boolean drawW, boolean drawU, boolean drawD, boolean drawInside)
    {
        ...
        if (drawD)
        {
            RenderHelper.DoQuadWithColor(vertexBuilder, matrix, BL, FL, FR, BR, UVPN, UVPP, UVNP, UVNN, light, color);
            if(drawInside)RenderHelper.DoQuadWithColor(vertexBuilder, matrix, BR, FR, FL, BL, UVPN, UVPP, UVNP, UVNN, light, color);
        }
        // min z
        if (drawN)
        {
    ...
    public static void renderCuboidWithDifferentSideUVs(Matrix4f matrix, VertexConsumer vertexBuilder, Vec3 pos, Vec3 size, float sizeShrink, float minU, float minV, float maxU, float maxV, float minUs, float minVs, float maxUs, float maxVs, int light,
                                    float r, float g, float b, float alpha, boolean drawN, boolean drawE, boolean drawS, boolean drawW, boolean drawU, boolean drawD, boolean drawInside)
    {
        ...
        Vec2 UVNP = new Vec2(minU, maxV);
        Vec2 UVPP = new Vec2(maxU, maxV);
        Vec2 UVNPs = new Vec2(minUs, maxVs);
        Vec2 UVPPs = new Vec2(maxUs, maxVs);
        Vec2 UVNNs = new Vec2(minUs, minVs);
        Vec2 UVPNs = new Vec2(maxUs, minVs);
        ...
        if (drawN)
        {
            RenderHelper.DoQuadWithColor(vertexBuilder, matrix, BR, BRT, BLT, BL, UVPPs, UVPNs, UVNNs, UVNPs, light, color);
            if(drawInside)RenderHelper.DoQuadWithColor(vertexBuilder, matrix, BL, BLT, BRT, BR, UVPPs, UVPNs, UVNNs, UVNPs, light, color);
        }
        // max z
        if (drawS)
        {
            RenderHelper.DoQuadWithColor(vertexBuilder, matrix, FL, FLT, FRT, FR, UVPPs, UVPNs, UVNNs, UVNPs, light, color);
            if(drawInside)RenderHelper.DoQuadWithColor(vertexBuilder, matrix, FR, FRT, FLT, FL, UVPPs, UVPNs, UVNNs, UVNPs, light, color);
        }
        // min x
        if (drawW)
        {
            RenderHelper.DoQuadWithColor(vertexBuilder, matrix, BL, BLT, FLT, FL, UVPPs, UVPNs, UVNNs, UVNPs, light, color);
            if(drawInside)RenderHelper.DoQuadWithColor(vertexBuilder, matrix, BLT, BL, FL, FLT, UVPPs, UVPNs, UVNNs, UVNPs, light, color);
        }
        // max x
        if (drawE)
        {
            RenderHelper.DoQuadWithColor(vertexBuilder, matrix, FR, FRT, BRT, BR, UVPPs, UVPNs, UVNNs, UVNPs, light, color);
            if(drawInside)RenderHelper.DoQuadWithColor(vertexBuilder, matrix, FRT, FR, BR, BRT, UVPPs, UVPNs, UVNNs, UVNPs, light, color);
        }
    }
